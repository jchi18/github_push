...
def github_get_repo_files(body: RepoFilesRequest):
    ...
    try:
        ...
        tree_url = f"https://api.github.com/repos/{body.repo_name}/git/trees/{branch_sha}?recursive=1"
        tree_response = requests.get(tree_url, headers=headers)
        
        # Handle empty repositories
        if tree_response.status_code == 404:
            print(f"Tree not found for branch {body.branch}, might be empty")
            return RepoFilesResponse(files=[])
        
        if tree_response.status_code != 200:
            raise HTTPException(status_code=400, detail=f"Failed to get tree: {tree_response.json().get('message', 'Unknown error')}")
        
        # Filter for relevant paths and fetch their content
        repo_files = []
        for item in tree_response.json()["tree"]:
            if item["type"] == "blob" and (
                item["path"].startswith("src/app/apis/") or
                item["path"].startswith("ui/src/components/") or
                item["path"].startswith("ui/src/pages/") or
                item["path"].startswith("ui/src/utils/")
            ):
                # Get file content
                blob_url = f"https://api.github.com/repos/{body.repo_name}/git/blobs/{item['sha']}"
        ...
        if is_empty_repo or not branch_sha:
            ...
            for file_path in body.files:
                try:
                    ...
                    # Add to new tree with normalized path
                    github_path = file_path.replace('/app/', '') if file_path.startswith('/app/') else file_path
                    tree_entry = {
                        "path": github_path,
                        "mode": "100644",
                        "type": "blob",
